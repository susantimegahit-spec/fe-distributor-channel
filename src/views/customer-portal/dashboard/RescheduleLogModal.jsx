...
const getLogParticipant = (log) => {
  ...
  const action = String(textValue(log.action, log.status, log.event))
    .toUpperCase()
    .replaceAll('_', ' ');
...
export default function RescheduleLogModal({ order, onClose, onSuccess }) {
  ...
  return (
    <Modal show onHide={onClose} centered dialogClassName="vendor-rates-approval-dialog reschedule-negotiation-dialog">
      <Modal.Header closeButton className="reschedule-negotiation-header">
        <div>
          <Modal.Title>Reschedule Negotiation</Modal.Title>
          <small className="text-muted">
            SO {order.order_number || order.sap_doc_num || order.doc_num || order.order_no || id} ·{' '}
            {order.customer_name || order.customer || '-'}
          </small>
        </div>
      </Modal.Header>
      <Modal.Body className="reschedule-negotiation-body">
        {loading ? (
          ...
          <Alert variant="danger">
            {error}{' '}
            <Button variant="link" onClick={() => setAttempt((value) => value + 1)}>
              Retry
            </Button>
          </Alert>
        ) : !logs.length ? (
          <p className="text-center text-muted py-5">No negotiation history yet.</p>
        ) : (
          <ol className="list-unstyled mb-0 w-100" aria-label="Negotiation history">
            {logs.map((log, index) => {
              ...
              return (
                <li
                  key={log.id ?? index}
                  className={`d-flex w-100 mb-3 ${isRightAligned ? 'justify-content-end' : 'justify-content-start'}`}
                >
                  <div className={`reschedule-chat-bubble palette-${participantIndex % 4} ${isRightAligned ? 'is-right' : 'is-left'}`}>
                    <div className="reschedule-chat-sender fw-semibold">{sender}</div>
                    {action && (
                      <small className="reschedule-chat-action d-block mb-2 fw-semibold">{String(action).replaceAll('_', ' ')}</small>
                    )}
                    {notes && <div style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{notes}</div>}
                    {details.proposed_delivery_date && (
                      <div className="mt-2">
                        <strong>Proposed Loading:</strong> {dateLabel(details.proposed_delivery_date)}
                      </div>
                    )}
                    {details.proposed_eta_date && (
                      <div>
                        <strong>Proposed ETA:</strong> {dateLabel(details.proposed_eta_date)}
                      </div>
                    )}
                    <small className="d-block text-muted mt-2">
                      {log.created_at && moment(log.created_at).isValid() ? moment(log.created_at).format('DD MMM YYYY · HH:mm') : '-'}
                    </small>
                  </div>
                </li>
              );
