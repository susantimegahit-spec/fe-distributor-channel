    ...
    <>
      <Stack direction="horizontal" gap={expanded ? 2 : 1} className="justify-content-end flex-wrap">
        <Button
          ...
          aria-label={`Approve order ${number}`}
          disabled={!id || saving}
        ...
        <Button
          ...
          aria-label={`Reschedule order ${number}`}
          disabled={!id || saving}
      ...
      <Modal show={Boolean(action)} onHide={() => !saving && setAction(null)} centered>
        <Form onSubmit={submit}>
          <Modal.Header closeButton={!saving}>
            <Modal.Title>
              {action === 'approve' ? 'Approve Order for Packing' : 'Reschedule Order'} {number}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {action === 'approve' ? (
              <p>Are you sure you want to approve order {number} for packing?</p>
            ) : (
              <>
                <Form.Group className="mb-3" controlId={`reschedule-eta-${id}`}>
                  ...
                  <ThemedDatePicker
                    value={form.eta}
                    disabled={saving}
                    onChange={(value) => setForm((current) => ({ ...current, eta: value }))}
                  />
                </Form.Group>
                <Form.Group className="mb-3" controlId={`reschedule-loading-${id}`}>
                  ...
                  <ThemedDatePicker
                    value={form.loading}
                    disabled={saving}
                    onChange={(value) => setForm((current) => ({ ...current, loading: value }))}
                  />
                </Form.Group>
